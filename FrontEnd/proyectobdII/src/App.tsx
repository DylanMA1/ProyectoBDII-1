...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      // Verificar el tipo de base de datos seleccionada para usar el endpoint correcto
      const endpoint =
        dbType === "postgresql"
          ? "http://localhost:5000/api/set-connection-postgresql"
          : dbType === "mysql"
          ? "http://localhost:5000/api/set-connection-mysql"
          : "http://localhost:5000/api/set-connection-sqlserver"; // Endpoint para SQL Server
      ...
      setConnectionUrl(
        dbType === "postgresql"
          ...
          : dbType === "mysql"
          ? "http://localhost:5000/api/mysql-data"
          : "http://localhost:5000/api/sqlserver-data" // URL de SQL Server
      );
      setConnected(true);
