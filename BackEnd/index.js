...
const { Pool } = require('pg');
const mysql = require('mysql2');
const sql = require('mssql'); // Importar el paquete para SQL Server
const cors = require('cors');
...
let pgPool;
let mysqlConnection;
let sqlServerPool; // Pool de conexión para SQL Server
...
app.post('/api/set-connection-sqlserver', (req, res) => {
  ...
  const config = {
    ...
    port: parseInt(port, 10),
    options: {
      encrypt: true, // Para Azure u otras implementaciones seguras
  ...
  sql.connect(config, (err) => {
    if (err) {
      ...
      console.log('Conectado a SQL Server');
      sqlServerPool = new sql.ConnectionPool(config); // Guardar la conexión en el pool
      res.send('Conexión a SQL Server establecida');
    }
  });
});
...
app.get('/api/postgresql-data', (req, res) => {
  ...
  Promise.all([pgPool.query(columnsQuery), pgPool.query(foreignKeysQuery)])
    .then(([columnsResult, foreignKeysResult]) => {
      res.json({
        columns: columnsResult.rows,
        foreignKeys: foreignKeysResult.rows,
...
app.get('/api/sqlserver-data', (req, res) => {
  ...
  Promise.all([
    sqlServerPool.request().input('database', sql.VarChar, database).query(columnsQuery),
    sqlServerPool.request().input('database', sql.VarChar, database).query(foreignKeysQuery),
  ])
    .then(([columnsResult, foreignKeysResult]) => {
      res.json({
        columns: columnsResult.recordset,
